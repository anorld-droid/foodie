class Strings {
  // Sign up screen strings
  final String createAccount = "Create Account";
  final String username = "Username";
  final String phoneNumber = "Phone Number";
  final String email = "Email";
  final String website = "Social Media handle";
  final String bio = "Bio";
  final String biometrics = "Register your Biometrics";
  final String alreadyHaveAnAccount = "Already have an account?";
  final String logIn = "Log in";
  final String slideSignUp = "Slide to sign up";
  //Biometrics
  final String regFace = "Register face";
  final String faceRecog = "Face Recognition";
  final String addFing = "Add fingerprint";
  final String scanFing = "Scan your fingerprint";

  //  Sign in screen strings
  final String enterusername = "Enter your username";
  final String contin = "Continue";
  final String welcome = "Welcome back";
  final String changeUsername = "Slide to change username";
  final String dontHaveAccount = "Don't have an account?";
  final String signup = "Sign up";

  //Top bar strings
  final String appName = "Trice";

  //Bottom bar
  final String news = "News Room";
  final String myApartment = "My Apartment";
  final String tasks = "Tasks";
  final String events = "Events";
  final String trending = "Trending";
  final String todoSvg = "assets/icons/todo.svg";

  //News Room
  final String headline = "News Brief";

  //News Details
  final String comments = "Comment";
  final String commentSvg = "assets/icons/comments.svg";
  final String toolTip = "You can only comment after reading the whole post.";
}
